...
export const NFTMarketplaceProvider = ({ children }) => {
  ...
  const createSeaportListing = async (tokenId, priceEth) => {
    try {
      ...
      const endTime = startTime + 30*24*60*60; // 30d
  ...
  // FINAL CORRECTED buyNFTSeaport function
  const buyNFTSeaport = async (order) => {
    try {
      ...
      // Properly reconstruct BigNumbers from database
      const reconstructBigNumber = (value) => {
        ...
        if (value._hex) {
          return ethers.BigNumber.from(value._hex);
        }
        if (value.hex) {
        ...
        if (value.value) {
          if (value.value.hex) {
            return ethers.BigNumber.from(value.value.hex);
          }
          if (value.value._hex) {
            return ethers.BigNumber.from(value.value._hex);
          }
      ...
      // Extract the values from the original order
      const priceInWei = reconstructBigNumber(order.parameters.consideration[0].startAmount);
      const tokenId = reconstructBigNumber(order.parameters.offer[0].identifierOrCriteria);
      const nftToken = order.parameters.offer[0].token;
      
      // Verify buyer has enough ETH
      const buyerBalance = await provider.getBalance(buyerAddress);
      if (buyerBalance.lt(priceInWei)) {
        throw new Error(
          `Insufficient ETH. Need ${ethers.utils.formatEther(priceInWei)} ETH`
        );
      }
      
      // CORRECTION CRITIQUE: Structure pour fulfillBasicOrder
      const basicOrderParameters = {
        // Ce que l'acheteur PAIE (ETH) - devient "consideration"
        considerationToken: ethers.constants.AddressZero, // ETH
        considerationIdentifier: ethers.BigNumber.from(0), // Not used for ETH
        considerationAmount: priceInWei, // Montant ETH à payer
        
        // Informations de l'ordre original
        offerer: order.parameters.offerer, // Le vendeur
        zone: order.parameters.zone || ethers.constants.AddressZero,
        
        // Ce que l'acheteur RECOIT (NFT) - devient "offer"
        offerToken: nftToken, // Contrat NFT (dynamique)
        offerIdentifier: tokenId, // Token ID
        offerAmount: ethers.BigNumber.from(1), // 1 NFT
        
        // Type d'ordre: ERC721_TO_ETH_FULL_OPEN = 1
        basicOrderType: 1,
        
        // Contraintes temporelles
        startTime: reconstructBigNumber(order.parameters.startTime),
        endTime: reconstructBigNumber(order.parameters.endTime),
        
        // Paramètres additionnels
        zoneHash: order.parameters.zoneHash || ethers.constants.HashZero,
        salt: reconstructBigNumber(order.parameters.salt),
        offererConduitKey: order.parameters.conduitKey || ethers.constants.HashZero,
        fulfillerConduitKey: ethers.constants.HashZero,
        
        // Pas de destinataires additionnels
      ...
      const tx = await seaportContract.fulfillBasicOrder(
